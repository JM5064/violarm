...
def initialize_mediapipe_hands(num_frames):
    ...
    for _ in range(num_frames):
        hand = mp_hands.Hands(
            ...
            max_num_hands=1, # Change to 1 or 2?
            model_complexity=1,
...
def process_frame(arm_model, hand_model, frame):
    ...
    returns:
        arm_keypoints: list[] of tensor([x, y]) arm keypoints   (reconsider this)
        hand_keypoints: list[] of tensor([x, y]) hand keypoints
    """
    ...
    if hand_results.multi_hand_landmarks:
        ...
        hand_keypoints.append(torch.Tensor([index.x * width, index.y * height]))
        hand_keypoints.append(torch.Tensor([middle.x * width, middle.y * height]))
        hand_keypoints.append(torch.Tensor([ring.x * width, ring.y * height]))
        hand_keypoints.append(torch.Tensor([pinky.x * width, pinky.y * height]))
    
    return list(arm_keypoints), list(hand_keypoints)
